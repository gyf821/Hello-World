...
马尔可夫过程$\mathrm{\{\xi(t)，t\inT\}}$可能取的值的全体组成过程的状态室间,$\mathrm{\xi(t)}$可能取的值称为状态。$\mathrm{\xi(t)=x}$代表在t时刻过程(或系统)外在状态x。马尔可夫过程的状态空间可以是连续的,也可以是离散的。马尔可夫过程的参数t可以是连续的,也可以是离散的。
...
$$
即在参数n=0,1,2,…,丑,状态取(0)=i,b(1)=1,(n-1)=in1,n)=i的条件下,点(n+1)=j的条件概率与(0),(1),(2),…,老(n-1)无关而仅与5(x)所取的值有关,把这类随机过程称为马尔可夫链。由定义可知P1(0)=动,占(1)=i1,与(2)=i,…,点n)=in}=P传(n)=in/(0)=i,(1)=i,…,(n-1)=in-1}·P传(0)=i,(1)=i1,…,5(-1s P5(n)=i/5(n-1)=in-1yP传(0)=i,(1)=i1,…,占(n-1)=in-}P店(n)=in/(n-1)=in-1}P{(n-1)=ia-/(n-2)=ias-2}P{(1)=i1/(0)=i}P{(0)=i}可见,为了描述马尔可夫链的(n+1)维概率分布,最重要的是条件概率P传(k+1)=j/(k)=i},称这条件概率为在时刻k时的一步转移概率p;(k)=P传(k+1)=正/(k)=它表示在时刻k时ξ(k)取i值的条件下,在下一时刻k+1时k+1)取1值的概率。显然P1(k)应具有以下两个性质(1)p(k)≥0(i,j∈I2)∑p(k)=1(i∈(三)齐次马尔可夫链定义如果在马尔可夫链中佔(k+1)=正/(k)=i即从i状态转移到状态的概率与k无关,则称这类马尔可夫链为齐次马尔可夫链。设P代表一步转移概率p所组成的矩阵,且状态空间↓由状态0,1,2,…所组成,则pPPi显然P矩阵的每个元素为非负的,并且每行之和均为1。P被称为步转移概率矩阵。如果状态空间的状态是有限的,则称为有限状态的马尔可大链;如果状态空间I是{0,±±2,…},则称为可列状态的马尔可夫链。