...
质点在直线上作随机游动。如某一时刻质点位于i,则下一步质点以概率p向右移动一格到达i+1。或以概率1-p=q向左移一格到达i-1。若以$\mathbf{\xi(n)}$表示时刻n时质点的位置,则{$\mathbf{\xi(n)}$,n=0,1,2,…}是一个随机过程。而且当$\mathbf{\xi(n)}$=i时,$\mathbf{\xi(n+1)}$,$\mathbf{\xi(n+2)}$,…,$\mathbf{\xi(n+k)}$,…等n时刻后质点所处的状态只与$\mathbf{\xi(n)}$=i有关,而与质点在n以前是如何到达i的完全无关。所以它是一个齐次马尔可夫链,其状态空间为{…,-2,-1,0,1,2,…},而其一步转移概率为